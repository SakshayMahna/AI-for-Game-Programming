...
int main() {
    sf::RenderWindow window(sf::VideoMode(800, 600), "Pocket Tanks");
    ...
    TargetIntelligence c2_aimer = TargetIntelligence();
    ...
    while (window.isOpen())
    {
        ...
        // Opponent Turn
            ...
            // Detect shoot
            if (!projectile_shot) {
                Steering c2_velocity = c2_aimer.calculated_aim(c1.getState(), c2.getState());
            ...
            if (projectile_shot) {
                ...
                if (c2_projectile->getHeight() > 3 * height / 4) {
                    ...
                    // Learn from mistake
                    c2_aimer.refine_targeting(width / 8, c2_projectile->getRange());
